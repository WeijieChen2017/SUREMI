...
model_E = Unet_softmax(unet_dict_E)
...
bin_loss = torch.nn.CrossEntropyLoss()
...
for idx_epoch_new in range(train_dict["epochs"]):
    ...
    for package in [package_train, package_val]:
        ...
        for cnt_file, file_path in enumerate(file_list):
            ...
            z_bon = np.asarray(batch_z>3/8).astype(int)
            z_sof_1 = batch_z<3/8
            z_sof_2 = batch_z>1/8
            ...
            if isTrain:
                ...
                optim.zero_grad()
                cls3_y_hat = model_E(cls3_z)
                loss = bin_loss(cls3_y_hat, cls3_y)
                loss.backward()
                optim.step()
