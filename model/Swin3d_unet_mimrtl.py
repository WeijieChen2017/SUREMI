...
class PatchUnMerging(nn.Module):
    ...
    def forward(self, x):
        ...
        new_x = torch.zeros([B, D*2, H*2, W*2, C//8], dtype=torch.float).cuda()

        new_x[:, 0::2, 0::2, 0::2, :] = x000
        new_x[:, 0::2, 0::2, 1::2, :] = x001
        new_x[:, 0::2, 1::2, 0::2, :] = x010
        new_x[:, 0::2, 1::2, 1::2, :] = x011
        new_x[:, 1::2, 0::2, 0::2, :] = x100
        new_x[:, 1::2, 0::2, 1::2, :] = x101
...
class SwinTransformer3D(nn.Module):
    ...
    def inflate_weights(self): # inflate_weights(self, logger):
        ...
        for k in relative_position_bias_table_keys:
            ...
            if nH1 != nH2:
                ...
                print("Error in loading {}, passing".format(k))
            else:
                if L1 != L2:
                    S1 = int(L1 ** 0.5)
        ...
        print("=> loaded successfully {}".format(self.pretrained))
        del checkpoint
    ...
    def init_weights(self, pretrained=None):
        ...
        if isinstance(self.pretrained, str):
            ...
            print("load model from: {}".format(self.pretrained))
