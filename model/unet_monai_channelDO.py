...
__all__ = ["UNet_channelDO"]


@export("monai.networks.nets")
@alias("Unet")
class UNet_channelDO(nn.Module):
    """
    ...
    def __init__(
        ...
        self.up3 = nn.Sequential(
                Convolution(3, self.channels[3]+self.channels[2], self.channels[1]*2, strides=self.strides[2],
        ...
        self.up2 = nn.Sequential(
                Convolution(3, self.channels[1]*2, self.channels[0]*2, strides=self.strides[1],
                kernel_size=self.up_kernel_size, act=self.act, norm=self.norm, dropout=self.dropout, 
                bias=self.bias, conv_only=False, is_transposed=True, adn_ordering=self.adn_ordering),
