...
def CM_sliding_window_inference(
    inputs: torch.Tensor,
    ...
    **kwargs: Any,
    ...
    Args:
        ...
        <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        cnt_sample: we plan to sample the whole model for many times and take median for robostness
        >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    for slice_g in tqdm(range(0, total_slices, sw_batch_size)) if progress else range(0, total_slices, sw_batch_size):
        ...
        seg_prob_out, CM = predictor(window_data, *args, **kwargs)             # batched patch segmentation

        if not reconOrCM:
            seg_prob_out = CM

        seg_prob_out.to(device)
